/**
 * Tests for getMediaByNarratorPage query function.
 */

import { getMediaByNarratorPage } from "@/db/library/get-media-by-narrator-page";
...
describe("getMediaByNarratorPage", () => {
  it("returns empty array when narrator has no media", async () => {
    const db = getDb();

    const narrator = await createNarrator(db);

    const result = await getMediaByNarratorPage(
      DEFAULT_TEST_SESSION,
      narrator.id,
      10,
    );
  ...
  it("returns media narrated by a narrator", async () => {
    const db = getDb();

    const narrator = await createNarrator(db, { name: "Stephen Fry" });
    const book1 = await createBook(db, { title: "Harry Potter 1" });
    const book2 = await createBook(db, { title: "Harry Potter 2" });
    ...
    expect(result[0]?.book.title).toBe("Harry Potter 2");
    expect(result[1]?.book.title).toBe("Harry Potter 1");
  });
  ...
  it("includes book with authors for each media", async () => {
    ...
    expect(result[0]?.book.authors[0]?.name).toBe("Test Author");
  });
  ...
  it("includes all narrators for each media", async () => {
    ...
    expect(result[0]?.narrators).toHaveLength(2);
  });
  ...
  // Note: pagination with publishedBefore uses raw SQL COALESCE which has
